...
if ( ! function_exists( 'cmc_posted_by' ) ) :
	/**
	 * Prints HTML with meta information about theme author.
	 */
	function cmc_posted_by() {
		printf(
		/* translators: 1: posted by label, only visible to screen readers. 2: author link. 3: post author. */
			'<span class="sr-only">%1$s</span><span class="author vcard"><a class="url fn n" href="%2$s">%3$s</a></span>',
			esc_html__( 'Posted by', 'cmc' ),
...
if ( ! function_exists( 'cmc_comment_count' ) ) :
	/**
	 * Prints HTML with the comment count for the current post.
	 */
	function cmc_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			/* translators: %s: Name of current post. Only visible to screen readers. */
			comments_popup_link( sprintf( __( 'Leave a comment<span class="sr-only"> on %s</span>', 'cmc' ), get_the_title() ) );
		}
	}
endif;

if ( ! function_exists( 'cmc_entry_meta' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 * This template tag is used in the entry header.
	 */
	function cmc_entry_meta() {
		...
		edit_post_link(
			sprintf(
				wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers. */
					__( 'Edit <span class="sr-only">%s</span>', 'cmc' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'cmc_comment_avatar' ) ) :
	/**
	 ...
	 */
	function cmc_get_user_avatar_markup( $id_or_email = null ) {

		if ( ! isset( $id_or_email ) ) {
			$id_or_email = get_current_user_id();
		}

		return sprintf( '<div class="vcard">%s</div>', get_avatar( $id_or_email, cmc_get_avatar_size() ) );
	}
endif;

if ( ! function_exists( 'cmc_discussion_avatars_list' ) ) :
	/**
	 * Displays a list of avatars involved in a discussion for a given post.
	 *
	 * @param array $comment_authors Comment authors to list as avatars.
	 */
	function cmc_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				cmc_get_user_avatar_markup( $id_or_email ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
		echo '</ol>', "\n";
	}
...
if ( ! function_exists( 'cmc_the_posts_navigation' ) ) :
	/**
	 * Wraps `the_posts_pagination` for use throughout the theme.
	 */
	function cmc_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => __( 'Newer posts', 'cmc' ),
				'next_text' => __( 'Older posts', 'cmc' ),
			)
		);
	}
endif;

if ( ! function_exists( 'cmc_content_class' ) ) :
	/**
	 ...
	 * CMC_TYPOGRAPHY_CLASSES constant.)
	 *
	 ...
	 */
	function cmc_content_class( $classes = '' ) {
		$all_classes = array( $classes, CMC_TYPOGRAPHY_CLASSES );
